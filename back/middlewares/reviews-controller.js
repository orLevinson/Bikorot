...
const editReview = async (req, res, next) => {
  ...
  let storedReview;
  try {
    storedReview = await Review.findById(reviewId);
  } catch (err) {
    const error = new HttpError("unknown error occured", 500);
    return next(error);
  }
  ...
  if (!storedReview) {
    const error = new HttpError("The specified review does not exist", 500);
    return next(error);
  }

  let hasUnitChanged = false;
  let newUnit;
  let previousUnit;
  ...
  if (!!reviewInfo.unit && !storedReview.unit._id.equals(reviewInfo.unit)) {
    ...
    // in case the info entered was incorrect there will be no unit
    // that match the one with the aforementioned echelons
    if (!newUnit) {
      const error = new HttpError("no such unit was found", 500);
      return next(error);
    }
  ...
  //  for calculating the new score we will be using the exact
  // way we used for the new review
  ...
  // edit the new data into the existing review

  if (!!hasUnitChanged) {
    storedReview.unit = newUnit._id;
    storedReview.command = newUnit.command;
    storedReview.division = newUnit.division;
  ...
  try {
    // in case the units have been changed
    // push the review to the new unit reviews array
    // and filter it out of the previous unit reviews array
    if (!!hasUnitChanged) {
      newUnit.reviews.push(storedReview._id);
      const reviewStr = storedReview._id.toString();
      previousUnit.reviews = previousUnit.reviews.filter((review) => {
        const currentStr = review.toString();
        return currentStr !== reviewStr;
      });
    }
    ...
    await sess.withTransaction(async () => {
      await Promise.all([
        storedReview.save({ session: sess }),
        // save the new and previous unit to DB only
        // if change have been made to them
        !!newUnit ? newUnit.save({ session: sess }) : null,
        !!previousUnit ? previousUnit.save({ session: sess }) : null,
      ]);
    });
    await sess.commitTransaction();
