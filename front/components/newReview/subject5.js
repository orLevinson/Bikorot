...
export default function Subject5(props) {
  const [categories, setCategories] = useState([]);
  const { reviewData, changeSubject5 } = useContext(reviewContextData);
  ...
  useEffect(() => {
    ...
    for (const [key, value] of Object.entries(questionsObj.subject5)) {
  ...
  const changeHandler = useCallback(
    (value, category, index, type) => {
      let copy = { ...reviewData.scores.subject5 };
      copy[category][index][
        type === "score" ? "score" : "text"
      ] = value;
      changeSubject5(copy);
    },
    [reviewData, changeSubject5]
  );

  if (!!reviewData.scores.subject5 && categories.length > 0) {
    return (
      <div>
                    ...
                    return (
                      ...
                      >
                        <Card>
                          <QuestionItem
                            valueScore={
                              reviewData.scores.subject5[question.category][
                                question.index
                              ].score
                            }
                            changeValueScore={(e) => {
